...
class Quiz:
    ...
    def wordAddButton(self):
        ...
        self.addPage.title("kelime ekleme")
        self.addPage.geometry("250x200")
        infoLabel = Label(self.addPage,text="Kelime girin",font="Times 20",width=17,bg="red")

        englishLabel = Label(self.addPage,text="English",font="Times 12", width=5)
        turkıshLabel = Label(self.addPage, text="Turkish", font="Times 12", width=5)

        self.wordEntryEnglish = Entry(self.addPage)
        self.wordEntryTurkısh = Entry(self.addPage)

        # def add_word():
        #     word = Word(wordEntryTurkısh, wordEntryEnglish, self.wordDatabase.databaseCounter() + 1)
        #     self.wordDatabase.add_words(word)
        #     print("word has been added")

        acceptButton = Button(self.addPage,text="Accept",command=self.wordAddFunction, font="Times 20",bg="green",fg="black",padx=78)
    ...
    def wordAddFunction(self):
        ...
        print(f"{english}=={turkish}  eklendi")
        sleep(5)
    ...
    def display_prev_que(self):
        # IT'LL HOLD THE PREV QUESTION
        self.prev_que = self.word.english
